...
bool LocalisationRTLSPlugin::LocalisationRTLSPlugin::processRangingResult(
  ...
  if (status == RTLSRangingStatus::AVAILABLE) {
    range_observation =
        makeRangeObservation_(initiator_index, responder_index, rangingResult);
    auto range2d =
        computeRange2D_(initiator_index, responder_index, rangingResult);
    storeRange2D(initiator_index, responder_index, range2d);
    return true;
  } else {
    resetRange2D(initiator_index, responder_index);
    return false;
  }
}
