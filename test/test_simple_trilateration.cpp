...
TEST(TestRobotToHuman, testTrilateration2D2A) {
  ...
  romea::VectorOfEigenVector2d anchor_positions(2);
  ...
  Eigen::Vector2d tagEstimatedPosition =
      romea::SimpleTrilateration2D::compute(anchor_positions, ranges);
...
TEST(TestRobotToHuman, testTrilateration2D3AUp) {
  ...
  romea::VectorOfEigenVector2d anchor_positions(3);
